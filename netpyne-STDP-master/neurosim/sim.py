...
class NeuroSim:
  def __init__(self, dconf, use_noise=True, save_on_control_c=True):
    conn_seed = dconf['sim']['seeds']['conn']  # 从配置中获取种子
    random.seed(conn_seed)
    wandb.init(project="rl-stdp-cartpole-gym", name=f"rl-stdp-seed{conn_seed}", config=dconf)
    # self.wandb_run = wandb.run  # 保存 run 对象以供后续使用
    ...
    # all_gids = [cell.gid for cell in sim.net.cells if hasattr(cell, 'gid')]
    # for gid in all_gids:  # 假设 all_gids 是所有分配的 gid 列表
    #   print(f"Binding GID {gid} to rank {sim.rank}")
    ...
    if sim.rank == 0:
      ...
      if game_done:
        ...
        if len(sim.AIGame.count_steps) > ep_cnt:
          ...
          wandb.log({"median": eval_ep})

        last_steps = [k for k in sim.AIGame.count_steps if k != 0][-1]
        self.epCount.append(last_steps)
        wandb.log({"episode_steps": last_steps, "episode": self.current_episode})
