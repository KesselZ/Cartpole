...
def _evaluate_step(pop: PopulationState, runner: RunnerState, conf: ESConfig,env_params:Any) -> PopulationState:
    ...
    print("act before", act, act.shape)
    act = jnp.argmax(act, axis=-1)
    act = act.astype(jnp.int32)
    print("act now", act, act.shape)
...
def main(conf):
    ...
    print("Network output dims:", network.out_dims)
...
def sweep(seed: int, conf_override: OmegaConf):
    def _objective(trial: optuna.Trial):
        conf = OmegaConf.merge(conf_override, {
            ...
            "project_name": f"E-SNN-acrobot-resevior",
